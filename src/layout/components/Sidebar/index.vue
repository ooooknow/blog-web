<template>
    <div>
        <el-scrollbar wrap-class="scrollbar-wrapper">
            <el-menu :default-active="activeMenu" :collapse="isCollapse" :background-color="variables.menuBg" :text-color="variables.menuText" :unique-opened="false" :active-text-color="variables.menuActiveText" :collapse-transition="false" mode="vertical">
                <sidebar-item v-for="route in routers" :key="route.path" :item="route" :base-path="route.path" />
            </el-menu>
        </el-scrollbar>
        <div class="submenu-sidebar" @click="toggleSideBar">
            <div class="hamburger" style="padding: 0 15px;">
                <img v-if="!sidebar.opened" src="~@/assets/images/open.png" alt="open" />
                <img v-if="sidebar.opened" src="~@/assets/images/draw.png" alt="draw" />
                <span>收起侧边栏</span>
            </div>
        </div>
    </div>
</template>

<script>
import { mapGetters } from 'vuex';
import SidebarItem from './SidebarItem';
import variables from '@/styles/variables.scss';

export default {
    components: { SidebarItem },
    computed: {
        ...mapGetters(['sidebar', 'routers']),
        // routes() {
        //     return this.$router.options.routes;
        // },
        activeMenu() {
            const route = this.$route;
            const { meta, path } = route;
            // if set path, the sidebar will highlight the path you set
            if (meta.activeMenu) {
                return meta.activeMenu;
            }
            return path;
        },
        variables() {
            return variables;
        },
        isCollapse() {
            return !this.sidebar.opened;
        },
    },
    methods: {
        toggleSideBar() {
            this.$store.dispatch('app/toggleSideBar');
        },
    },
};
</script>

<style lang="scss" scoped>
.submenu-sidebar {
    position: absolute;
    left: 0;
    bottom: 0;
    height: 56px;
    line-height: 56px;
    width: 100%;
    border-top: 1px solid #e5e5e5;
    .hamburger {
        cursor: pointer;
        img {
            display: inline-block;
            vertical-align: middle;
            width: 14px;
        }
        span {
            color: #707070;
            font-size: 14px;
            vertical-align: middle;
            margin: 0 10px;
        }
        &:hover {
            background-color: #fff3ec;
        }
    }
}
</style>
